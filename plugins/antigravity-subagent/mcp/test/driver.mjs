...
`, 'utf8');
  ...
  assert.equal(
    buildAgyStreamUserMessage('hello'),
    JSON.stringify({ event: 'user', message: { content: 'hello' } }),
  );
  ...
  assert.equal(second.result?.conversationId, 'driver-conversation-1');
  assert.equal(second.result?.response, 'second');
