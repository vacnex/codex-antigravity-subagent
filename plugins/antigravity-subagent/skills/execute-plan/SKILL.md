...
3. if the passive wait interval genuinely ends or the transport is interrupted while the logical PLAN worker is still active, you may call `agy_wait` at most ONCE more (maximum 2 `agy_wait` calls total for a single turn);
4. do not enter an open-ended wait loop: if the worker does not finish after 2 wait calls, call `agy_status` once to inspect progress and git diff. If no changes have been made or the worker is stalled, report the situation to the user immediately rather than burning supervisor tokens with repeated wait calls;
...
for a real transport interruption or a wait interval that actually expired, capped at 2 calls total.
...
Do not orchestrate long work with shell sleeps, frequent `agy_status`, or model-driven timeout recovery loops. Never call `agy_wait` more than twice per turn.
